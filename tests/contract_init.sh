...
echo "Step 1: Run graph node contracts"
...
GRAPH_NODE=$(python obtain_address.py "$FULL_CMD_LOG")
...
ISTAKING_VAR=$(forge create --unlocked --from $GATEWAY --rpc-url localhost:8545 test/MockStaking.sol:MockStaking --constructor-args $GRAPH_NODE --json)
...
ESCROW_VAR=$(forge create --unlocked --from $GATEWAY --rpc-url localhost:8545 src/Escrow.sol:Escrow --constructor-args $GRAPH_NODE $ISTAKING_AD $TAP_VERIFIER_AD $ALLOCATION_TRACKER_AD 10 15 --json)
ESCROW_AD=$(echo $ESCROW_VAR | jq -r '.deployedTo')
echo "Escrow address: $ESCROW_AD"

echo "Approving escrow contract"
cast send --unlocked --from $GATEWAY $GRAPH_NODE 'approve(address spender, uint256 amount)' $ESCROW_AD '10000'
cast send --unlocked --from $SIGNER $GRAPH_NODE 'approve(address spender, uint256 amount)' $ESCROW_AD '10000'
cast send --unlocked --from $SIGNER $GRAPH_NODE 'approve(address spender, uint256 amount)' $ESCROW_AD '10000'
cast send --unlocked --from $GATEWAY $GRAPH_NODE 'setAssetHolder(address, bool)' $ESCROW_AD true
# Making sure other accounts have money
cast send --unlocked --from $GATEWAY $GRAPH_NODE 'transfer(address to, uint256 amount)' $SIGNER 500
cast send --unlocked --from $GATEWAY $GRAPH_NODE 'transfer(address to, uint256 amount)' $RECEIVER 500
